...
namespace stm_controller
{
    public partial class Form1 : Form
    {
        ...
        static string resp;
        static bool enRespToPkt = false;
        static bool enInterceptor = false;
        static string interceptorPacket;

        // Class Declaration
        static TransmitPacket PacketHandler;
        ...
        public Form1()
        {
            ...
            PacketHandler = new TransmitPacket();
        ...
        private void keyCaptureSendData_Click(object sender, EventArgs e)
        {
            if (!send_data_flag)
            {
                tx_repeater_delay.Interval = (int)send_delay.Value;
         ...
         *  S.Aman
         */
        private void tx_packet_radioButton_CheckedChanged(object sender, EventArgs e)
        {
            txPktSettings tx_pkts = new txPktSettings();
            tx_textarea.Clear();
            tx_pkts.ShowDialog();
        }
         ...
         *  S.Aman
         */
        private void keyCapture_tx_textarea_KeyPress(object sender, KeyPressEventArgs e)
        {
        ...
        private void rx_data_event(object sender, System.IO.Ports.SerialDataReceivedEventArgs e)
        {
            if (serialPort1.IsOpen)
            {
                ...
                this.BeginInvoke((Action)(() =>
                {
                    ...
                    if (enRespToPkt)
                    {
                        processPacket(data_rx);
                        serialPort1.Write(resp);
                    }

                    if (enInterceptor)
                    {
                        processInterceptor(data_rx); 
                    }
        ...
        public static void processPacket(string packet)
        {
            ResponsePacket decodedData;

            decodedData = new ResponsePacket();

            resp = decodedData.decodeResponse(packet);
        }

        public static void processInterceptor(string packet)
        {
            GenerateInterceptor interceptorData;

            interceptorData = new GenerateInterceptor();

            interceptorPacket = interceptorData.Response(packet);
        }
        ...
        private void update_rxtextarea_event(object sender, DoWorkEventArgs e)
        {
            ...
            this.BeginInvoke((Action)(() =>
            {
                ...
                if (enRespToPkt)
                {
                    if (errorCode == 1)
                    {
                        main_textBox_binary.AppendText("" + DateTime.Now + " ZOOM not found\n");
                    }
                    else
                    {
                        main_textBox_binary.AppendText("" + DateTime.Now + " ZOOM found\n");
                    }
                }

                if (enInterceptor)
                {
                    main_textBox_binary.AppendText("" + DateTime.Now + interceptorPacket + "\n");
                    serialPort1.Write(interceptorPacket);
                }
        ...
        private void txPacketSendButton_Click(object sender, EventArgs e)
        {
            //string tx_data = string.Empty;
            string tx_data = "";
            string convertedData;

            tx_data = txPacketTextBox.Text;
            convertedData = ConvertAsciiToBinary(tx_data);
            tx_data = PacketHandler.encodeCommand(convertedData);
            ...
            /*tx_data = txPacketTextBox.Text.Replace("\n", Environment.NewLine);
            serialPort1.Write(txPktSet.SOT);
            serialPort1.Write(tx_data.Replace("\\n", Environment.NewLine));*/
        }

        private void sendPacket(string packetData)
        {
            serialPort1.Write(packetData);
        }

        private void txPktSettingsButton_Click(object sender, EventArgs e)
        {
            txPktSettings a = new txPktSettings();
            a.ShowDialog();
        }

        private void respToPktChkBox_CheckedChanged(object sender, EventArgs e)
        {
            if (enRespToPktChkBox.Checked)
            {
                enRespToPkt = true;
            }
            else
        ...
        private void enInterceptorChkBox_CheckedChanged(object sender, EventArgs e)
        {
            if (enInterceptorChkBox.Checked)
            {
                enInterceptor = true;
            }
            else
         ...
         *  tx_data
         * @brief 
         *  This function convert Ascii characters into binary.
         * @author  
         *  S.Aman
         */
        private string ConvertAsciiToBinary(string tx_data)
        { 
            string binarystring = String.Join(String.Empty, tx_data.Select( c => Convert.ToString(Convert.ToInt32(c.ToString(), 16), 2).PadLeft(4, '0')));
            //output = string.Join(" ", tx_data.Split(new[] { ' ' }, StringSplitOptions.RemoveEmptyEntries).Select(s => Convert.ToString("0123456789ABCDEF".IndexOf(s, StringComparison.InvariantCultureIgnoreCase), 2)).Select(s => s.PadLeft(4, '0')));
